...
<head>
    ...
    <title>The Wedding of {{ $data['groom']['nickname'] ?? 'Groom' }} & {{ $data['bride']['nickname'] ?? 'Bride' }}</title>
    <!-- Tailwind CSS -->
...
<body class="antialiased bg-gray-100 flex justify-center w-full min-h-screen">
    ...
    <div x-data="invitationData()" class="w-full lg:w-[480px] h-[100dvh] bg-white relative overflow-x-hidden lg:shadow-2xl font-montserrat bg-forest" style="transform: translateZ(0);">
                ...
                <div class="w-40 h-40 sm:w-48 sm:h-48 border border-[#7a7a7a]/40 rounded-full flex items-center justify-center mb-8 p-2">
                    <div class="w-full h-full border border-[#31626c]/30 rounded-full flex items-center justify-center bg-white/50 backdrop-blur-sm">
                        <div class="text-center">
                            ...
                            <h1 class="text-3xl sm:text-4xl font-cinzel font-semibold text-black">{{ substr($data['bride']['nickname'] ?? 'B', 0, 1) }} & {{ substr($data['groom']['nickname'] ?? 'G', 0, 1) }}</h1>
                        </div>
                    </div>
                </div>

                <p class="text-xs uppercase tracking-[0.2em] font-montserrat text-gray-500 mb-2">The Wedding Of</p>
                <h1 class="text-5xl sm:text-6xl font-script mb-8 text-black drop-shadow-sm">{{ $data['bride']['nickname'] ?? 'Bride' }} & {{ $data['groom']['nickname'] ?? 'Groom' }}</h1>
        ...
        <div class="w-full h-full overflow-y-auto overflow-x-hidden relative">
            ...
            <section class="min-h-[85vh] flex flex-col items-center justify-center p-8 text-center relative pt-20">
                ...
                <h1 class="text-5xl font-script text-black mb-4">{{ $data['bride']['nickname'] ?? 'Bride' }} & {{ $data['groom']['nickname'] ?? 'Groom' }}</h1>
                <div class="text-sm font-cinzel font-semibold tracking-widest text-black border-t border-b border-gray-300 py-3 px-6 mt-2">
                    {{ $data['event']['date'] ?? 'Coming Soon' }}
                </div>
            </section>
            ...
            <section class="py-24 px-6 text-center">
                <div class="flex flex-col gap-16 items-center">
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <h3 class="text-2xl font-cinzel font-bold text-black mb-2">{{ $data['bride']['name'] ?? 'The Bride' }}</h3>
                        <p class="text-[11px] text-gray-500 tracking-wide max-w-[250px] mx-auto leading-relaxed">{{ $data['bride']['parents'] ?? 'Putri dari ...' }}</p>
                    </div>
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <h3 class="text-2xl font-cinzel font-bold text-black mb-2">{{ $data['groom']['name'] ?? 'The Groom' }}</h3>
                        <p class="text-[11px] text-gray-500 tracking-wide max-w-[250px] mx-auto leading-relaxed">{{ $data['groom']['parents'] ?? 'Putra dari ...' }}</p>
                    </div>
                </div>
            </section>
            ...
            <footer class="py-20 text-center bg-[#111] text-white">
                ...
                <h3 class="text-4xl font-script">{{ $data['bride']['nickname'] ?? 'Bride' }} & {{ $data['groom']['nickname'] ?? 'Groom' }}</h3>
            </footer>
    ...
    <script>
        function invitationData() {
            return {
                isOpen: {{ isset($is_preview) && $is_preview ? 'true' : 'false' }},
                isPlaying: false,
                openInvitation() {
                    this.isOpen = true;
                    this.isPlaying = true;
